...
- [ ] 7. Implement all 245 base opcodes in the match block inside `Cpu::step`. Organize as comments by group. Key implementation notes:
    - **0x00 NOP**: 4 cycles.
    ...
    - **DAA (0x27)**: explicit algorithm — if N flag clear (last op was addition): if C set or `A > 0x99` → `A = A.wrapping_add(0x60)`, set C; if H set or `(A & 0x0F) > 0x09` → `A = A.wrapping_add(0x06)`. If N flag set (last op was subtraction): if C set → `A = A.wrapping_sub(0x60)`; if H set → `A = A.wrapping_sub(0x06)`. Always: Z = (A==0), H = 0; C unchanged except where set above.
    - **0xE8 ADD SP, e** and **0xF8 LD HL, SP+e**: `e` is a signed 8-bit immediate; reinterpret it as `e_u8 = e as u8`. Flags: Z=0, N=0; H = `((sp & 0xF) + (e_u8 & 0xF)) > 0xF`; C = `((sp & 0xFF) + e_u8 as u16) > 0xFF`. Result: `sp.wrapping_add(e as i8 as i16 as u16)`. For `LD HL, SP+e` the result goes into HL, SP is unchanged.
    - **LDH** (0xE0/0xF0): address = 0xFF00 + immediate byte.
    - **LD (C) / LD A,(C)** (0xE2/0xF2): address = 0xFF00 + C.
    - **LD (HL+/HL-)** (0x22/0x32/0x2A/0x3A): HL post-increment/decrement.
    - **PUSH/POP AF**: set_af must mask F lower nibble to 0.
    - **RST n** (0xC7..0xFF every 8): CALL to 0x00/0x08/0x10/0x18/0x20/0x28/0x30/0x38.
    - **Undefined opcodes**: `panic!("undefined opcode {:#04X} at {:#06X}", opcode, self.pc.wrapping_sub(1))`.
